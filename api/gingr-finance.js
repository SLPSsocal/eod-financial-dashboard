...
module.exports = async function handler(req, res) {
  ...
  try {
    const ids = await fetchInvoiceIds(config.subdomain, config.key, from_date, to_date);
    const transactions = await batchFetch(config.subdomain, config.key, ids);
    const totals = aggregate(transactions);
    const net = Math.round(Object.values(totals).reduce((a, b) => a + b, 0) * 100) / 100;
    return res.status(200).json({ success: true, facility, facilityName: config.name, from_date, to_date, invoice_count: ids.length, totals, net_total: net });
  } catch (err) {
